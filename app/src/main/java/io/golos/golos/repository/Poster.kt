        ...
        try {
            ...
            (0 until content.size)
                    .forEach {
                        ...
                        if (part is EditorImagePart) {
                            val newUrl = if (part.imageUrl.startsWith("/") || part.imageUrl.startsWith("file")) mGolosApi.uploadImage(mRepository.appUserData.value?.name!!,
                                    File(part.imageUrl))
                            else part.imageUrl
            ...
            val result = when (type) {
                ...
                EditType.CREATE_POST -> {
                    ...
                    mGolosApi.sendPost(mRepository.appUserData.value?.name!!,
                ...
                EditType.EDIT_POST -> {
                    ...
                    mGolosApi.editPost(originalPostOrCommentPermlink, mRepository.appUserData.value?.name!!,
                            title, content, tags.toArrayList().toArray(Array(tags.size, { "" })))
                }
                EditType.CREATE_COMMENT -> {
                    ...
                    mGolosApi.sendComment(mRepository.appUserData.value?.name!!,
                ...
                EditType.EDIT_COMMENT -> {
                    ...
                    mGolosApi.editComment(mRepository.appUserData.value?.name!!,
