    ...
    fun testAuth() {
        ...
        repo.authWithPostingWif(userName, postingWif = privatePosting,
                listener = {
                    ...
                    Assert.assertEquals(userName, resp.accountInfo.golosUser.userName)
                    Assert.assertNotNull(resp.accountInfo.golosUser.avatarPath)
                    Assert.assertNotNull(resp.accountInfo.userMotto)
                    Assert.assertTrue(resp.accountInfo.postsCount != 0L)
                    Assert.assertTrue(resp.accountInfo.subscribesCount != 0L)
                    ...
                    Assert.assertTrue(resp.accountInfo.subscribesCount > resp.accountInfo.subscribersCount)
                })
